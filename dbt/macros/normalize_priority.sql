...
{% macro normalize_priority(col) %}
    case
        -- Nhóm 1 — nguồn gửi số theo contract cũ. Chỉ nhận đúng miền 1..4:
        -- try_cast một mình vẫn cho '0', '5', '-1' đi qua vì chúng là số hợp lệ.
        when try_cast({{ col }} as integer) between 1 and 4
            then try_cast({{ col }} as integer)

        -- Nhóm 2 — nhãn chữ dùng từ 2026-08-10. Ý nghĩa không đổi, chỉ đổi
        -- cách biểu diễn, nên quy về số theo tài liệu API thay vì loại bỏ.
        ...
        -- NULL là tín hiệu để quarantine_tickets nhặt bản ghi ra.
        else null
    end
{% endmacro %}
...
{% macro priority_reject_reason(col) %}
    case
        ...
        else 'priority là chuỗi lạ, không có trong bảng quy đổi nhãn'
    end
{% endmacro %}
